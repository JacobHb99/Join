...
/**
 * Saves new substask from current task.
...
/**
 * Changes the image-path of the delete button.
 */
function changeImg() {
...
function renderContactsModifyAddTask(Id) {
    ...
    for (let i = 0; i < allContacts.length; i++) {
        ...
        if(newTaskArray[Id]['assignedTo'].includes(name)) {
            content.innerHTML += renderPopUpDisabledAssignment(color, name, Id)} else {
            content.innerHTML += renderPopUpAssignment(color, name, Id)
        }
    } 
}
...
/**
 * Pushes contacs to the assignedTo-Array, if they are not already existing.
 */
